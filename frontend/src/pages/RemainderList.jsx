...
const ReminderList = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/api/reminders").then((response) => {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-900">
            <div className="p-8 bg-gray-900 text-white rounded-xl shadow-lg border border-gray-700 max-w-md w-full mt-16">
                <h2 className="text-2xl font-bold mb-6 text-cyan-400 text-center">📅 Upcoming Reminders</h2>
                ...
                {reminders.length === 0 ? (
                    ...
                    <ul className="space-y-3">
                        {reminders.map((reminder, index) => (
                            ...
                            >
                                <span className="text-lg font-semibold text-white">💊 {reminder.medication}</span>
