...
const AppointmentForm = ({ doctor }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post("http://localhost:3000/api/appointments/appointments", appointmentData);
    ...
    return (
        <div className="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-md mt-6 text-black">
            <h2 className="text-2xl font-bold text-center text-gray-700">Book Appointment</h2>
            <form onSubmit={handleSubmit} className="mt-4">
                ...
                <select value={time} onChange={(e) => setTime(e.target.value)}
                    ...
                    {doctor?.availableSlots.map(slot => <option key={slot} value={slot}>{slot}</option>)}
                </select>
