...
const DoctorList = ({ onSelectDoctor }) => {
  ...
  useEffect(() => {
    axios
      .get(`${backendUrl}/api/doctor/doctors`)
      .then((response) => {
      ...
      {doctors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {doctors.map((doctor) => (
            <div
              key={doctor._id}
              className="relative bg-[#1E293B] p-6 rounded-xl shadow-lg hover:shadow-2xl transition duration-300 border border-gray-700"
            >
              {/* Doctor Image */}
              <div className="absolute top-[-40px] left-4 w-16 h-16 bg-gray-800 rounded-full flex items-center justify-center border border-gray-600">
                <img
                  src={`https://i.pravatar.cc/80?img=${Math.floor(
                    Math.random() * 70
